...
function L = getCholeskyDecomposition(A,tmp)
	...
	% Use original storage of A to save L.
% 	Columns = size(A,2);
% 	for col = 1:Columns
% 		diagSum = 0;
% 		for i = 1:col-1
% 			diagSum = diagSum + A(col,i)^2;
% 		end
% 
% 		%Attention that because use A to save, A(i,i) shouldn't be changed.
% 		factor = sqrt(A(col,col) - diagSum);		
% 
% 		recursiveValue = 0;
% 		for i = 1:col-1
% 			recursiveValue = recursiveValue + A(col,i).*A(:,i);
% 		end
% 		A(:,col) = (A(:,col) - recursiveValue) ./factor;
% 	end
% 
% 	L = A;
