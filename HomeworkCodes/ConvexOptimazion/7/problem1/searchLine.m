...
function stepSize = searchLine(function_fx, x0, dir)
...
while f_a > f_b			% left is larger than right, continue to find
	...
	b = gamma^i * delta;
	i = i+1;
	f_a = calDefaultFunctionValue(x0 + a*dir);
	f_b = calDefaultFunctionValue(x0 + b*dir);
...
while b-a > epsilon 
	value1 = calDefaultFunctionValue(x0 + s1*dir);
	value2 = calDefaultFunctionValue(x0 + s2*dir);
